...
class Controls:
  def __init__(self, CI=None):
    self.params = Params()
    self.disableNoGPSWarn = self.params.get_bool("DisableNoGPSWarn")
    ...
    if not SIMULATION or REPLAY:
      if not self.disableNoGPSWarn:
        ...
        if not self.sm['liveLocationKalman'].gpsOK and self.sm['liveLocationKalman'].inputsOK and (self.distance_traveled > 1500):
          print("gpsgpsgps")
          print("self.disableNoGPSWarn", self.disableNoGPSWarn)
          print("paramsGps", self.params.get_bool("DisableNoGPSWarn")) 
          self.events.add(EventName.noGps)
          if self.distance_traveled > 2000:
            self.disableNoGPSWarn = True
  ...
  def state_control(self, CS):
    ...
    if not self.joystick_mode:
      ...
      if self.model_use_lateral_planner:
        ...
        delay = self.CP.steerActuatorDelay if not hasattr(self.LaC, "enable_DSAD") or not self.LaC.enable_DSAD else self.LaC.dsad
        delay = delay + .2
  ...
  def publish_logs(self, CS, start_time, CC, lac_log):
    ...
    if CC.cruiseControl.cancel:
      print("canceling cruise control", CS.brakePressed, self.enabled_long, self.CS_prev.brakePressed, CS.standstill)
    ...
    if hasattr(controlsStateSP, "dynamicSteerActuatorDelay") :
        controlsStateSP.dynamicSteerActuatorDelay = self.CP.steerActuatorDelay if not hasattr(self.LaC, "enable_DSAD") or not self.LaC.enable_DSAD else self.LaC.dsad
